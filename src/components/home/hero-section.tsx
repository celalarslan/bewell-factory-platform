...
import { heroBullets, heroButtons, brandOperations } from "@/data/home";
    ...
    <section id="top" className="relative min-h-[78vh] overflow-hidden pt-20 md:min-h-[88vh]">
      <div className="absolute inset-0">
        <Image
          ...
          alt="Industrial project development hero background"
          fill
          priority
          sizes="100vw"
          className="object-cover object-[68%_center]"
        />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(8,11,15,0.96)_0%,rgba(8,11,15,0.86)_36%,rgba(8,11,15,0.56)_58%,rgba(8,11,15,0.14)_100%)]" />
      ...
      <div className="relative mx-auto grid min-h-[calc(78vh-80px)] max-w-[1440px] items-center px-5 py-14 md:min-h-[calc(88vh-80px)] md:py-16 lg:px-10">
        <div className="max-w-4xl">
          <p className="max-w-xl text-[11px] font-semibold tracking-[0.24em] text-[#d6d2cb] sm:tracking-[0.32em] md:text-[13px]">
            INDUSTRIAL PROJECT DEVELOPMENT & DELIVERY
          </p>
          <h1 className="mt-5 max-w-4xl text-[46px] font-semibold leading-[0.94] tracking-[-0.055em] text-white sm:text-5xl md:text-7xl xl:text-[92px]">
            From Industrial Vision
            <span className="block">
              to <span className="text-[#b21f24]">Operational Reality</span>
            </span>
          </h1>
          <p className="mt-7 max-w-2xl text-base leading-8 text-[#e7e4de] md:text-lg">
            Novertra structures, engineers and delivers industrial projects from initial concept to operational readiness,
            connecting project opportunities with engineering, equipment, construction and execution capability.
          </p>
          ...
          <div className="mt-10 grid gap-3 text-sm text-[#d8ddd8] sm:grid-cols-3">
            {heroBullets.map((bullet) => (
              <div key={bullet} className="border-l border-white/15 pl-4">
                {bullet}
              </div>
            ))}
          </div>

          <div className="mt-8 text-sm tracking-[0.18em] text-[#f2dfcf]">{brandOperations}</div>
        </div>
      </div>
    </section>
  );
